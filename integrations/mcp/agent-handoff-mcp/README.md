...
The wrapped CLI infers the sending session from `HANDOFF_SESSION_ID`, `CLAUDE_CODE_SESSION_ID`, `CODEX_SESSION_ID`, `COPILOT_SESSION_ID`, `GITHUB_COPILOT_SESSION_ID`, or a project-local fallback session. `handoff_inbox` also accepts `sessionId` to read a specific live session. Message tools address sessions with `@alias` or session IDs; `handoff_run` and `handoff_delegate` target delegation profiles.
